...
export class SideBarNav extends Component {
    ...
    renderWorkspaces(workspaces) {
        let content = workspaces.map(item => React.createElement(SideBarItem, { key: item.url, name: item.name, url: "/workspace" + item.reactLocalUrl, icon: item.icon, initials: item.initials, exactMatch: false, itemKey: item.url }));
        return (content);
    }
